...
function drop(ev) {
  ...
  if (dropBox.id.includes("_copy")) {
    //god I hate this, basically its setting the img as the target, this if statement corrects it to be the dropBox
    if (dropBox.id.includes("A")) {
      //we are at anion dropBox
      dropBox = document.getElementById("anion");
      //console.log("Checkpoint 1");
    } else {
      //we are at cation dropBox
      dropBox = document.getElementById("cation");
    }
  }

  if (dropBox.classList.item(1) == img.classList.item(1)) {
    //make sure img type matches dropBox type, anion or cation
    ...
    if (numElems == 0) {
      ...
      //there is 1 img already in dropBox
      ...
      if (existingImg.id.includes("A2") && img.id.includes("A2")) {
        ...
        existingImg.src = "images/compound_IMGS/NaOHxs.JPG";
        const rm = document.getElementById("A2_copy");
        //rm.remove();
        //console.log("Are you activating?");
        return;
      } else {
        //all other existingImgs just get replaced
